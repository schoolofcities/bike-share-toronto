<script>
	...
	<main>
		...
		<div class="text">
			<h2>Why & How</h2>
			<p>
				There has been substantial <a href="./growth">growth</a> in Bike Share ridership in Toronto over the past several years. We recently wanted to A) improve our understanding about where people use Bike Share and B) estimate the characteristics of these trips (network distance, speed, elevation gain/loss, etc.) to use in future analyses on how people cycle in the city.
			</p>
			<p>
			...
			<p>
				...
				We set out to estimate this route information with the help of open data and software. Once this was calculated, we layered the route for every trip onto a map to look at overall travel patterns.
			</p>
			<p>
				To do this, we needed the help of a <a href="https://en.wikipedia.org/wiki/Graph_(discrete_mathematics)" target="_blank">graph</a> representing the cycling network in the City of Toronto. We created this using network data from <a href="https://www.openstreetmap.org/#map=14/43.6586/-79.3833" target="_blank">OpenStreetMap</a> and <a href="https://www.openstreetmap.org/#map=14/43.6586/-79.3833" target="_blank">GraphHopper</a>, an open source network analysis software, to build a routable network graph that works off a local computer. Each line segment in the network is weighted by its length but also its attributes (e.g. designated cycling trails away from car, like the Waterfront Trail, are up-weighted; busy car-centric roads without cycling infrastructure are down-weighted). From this network graph we can make queries, similar to Google Maps, where we input the start location and end location, and ask the graph to return us the most plausible route along with information on trip duration, distance, and change in elevation. Here's an example for a short trip.
			</p>
